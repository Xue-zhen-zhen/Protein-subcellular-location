...
class MIL(object):
    def __init__(self, config: Config, shape):
        ...
        # tf Graph input
        X = tf.placeholder(tf.float32, [None, height, width, 2], name="Features")
        y_ = tf.placeholder(tf.float32, [None, n_classes], name="Labels")
        # dropout
        d = tf.placeholder(tf.float32)
...
def getPatchInput(config, X):
    ...
    input = tf.reshape(patches, [-1, config.patch_size, config.patch_size, config.get_value("n_channels", 2)])
