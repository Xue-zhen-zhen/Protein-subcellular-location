...
class ProteinLoc(TorchDataset):
    def __init__(self, data_directory_path: str , label_file: str = None, transforms=None, subset: float = 1.,
                 num_classes: int = 3, verbose: bool = False, patching: bool = False, patch_size: int = 112):   #zzxue
        """ Read samples from cyto dataset."""
        self.verbose = verbose
        self.patching = patching
        self.patch_size = patch_size
        self.n_classes = num_classes
        self.classes = ["Cytoplasmic&membranous", "Cytoplasmic&membranous&nuclear", "nuclear"]
        assert (os.path.exists(data_directory_path))
    ...
    def __getitem__(self, idx):
        ...
        if self.patching:
            ...
            sample['input'] = sample['input'].permute(
                ...
                -1, 2, self.patch_size, self.patch_size).permute(0, 2, 3, 1).numpy()  #zzxue
        else:
