function [mean_per_evalmodel,indices]=ten_foldValidation(Data,clabels,idx_sda) %  perprotein class,alabels
...
Data_labels = clabels;
[m,n] = size(Data);
indices=crossvalind('Kfold',Data(1:m,n),10);  %  perimages class
% indices = per_protein(alabels);  %  perprotein class
for k=1:10
    test_ind = (indices==k);
    train_ind = ~test_ind;
