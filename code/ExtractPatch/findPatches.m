function centers_pnas = findPatches(I,prot,nuc,radius,numRegions, knownCenters ,writepath,imagename)
...
%
centers = []; I_used = I; I_worked = I_used; I_used_0=I_used;
prot_orig = prot; nuc_orig = nuc;

if field_shape == 'circle'
    rad_disk= zeros(2*radius+1);
    rad_disk(radius+1,radius+1)=1;
    b=bwdist(rad_disk);
    rad_disk(find(b<radius))=1;
elseif field_shape == 'square'           
        rad_disk = zeros(2*radius+1)+1;
end
Region_coord = [];
for iR = 1:numRegions
    if iR<=size(knownCenters,1) % know the i-th center
        cen_x = knownCenters(iR,1);
        cen_y = knownCenters(iR,2);
    else
        ic = 0; ic2 = 0;
     ...
     x_coordinate(iR,1) = cen_x-radius;
     x_coordinate(iR,2) = cen_y-radius;
    %
    I_used_patch = I_used(cen_x-radius:cen_x+radius,cen_y-radius:cen_y+radius,:) .* repmat(uint8(rad_disk),[1,1,3]);
    Image_name = [imagename(1:end-4) '_(' num2str(iR) ').png'];
    if ~exist([writepath Image_name],'file')
       imwrite(I_used_patch,[writepath Image_name]);
    end
    %
...
%  I_used_1 = drawRectangleFrame(I_used_0,x_coordinate,[2*radius+1,2*radius+1]);
